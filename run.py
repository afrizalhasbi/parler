...
def main():
        ...
        for split in vectorized_datasets:
            ...
            if accelerator.is_main_process and len(all_generated_labels) > 0:
                ...
                tmp_labels = tmp_labels.map(
                    ...
                    num_proc=data_args.preprocessing_num_workers,  # this one is resource consuming if many processor.
                    input_columns=["labels"],
                    desc="Postprocessing labeling (modified to only use 1 worker)",
                )
    ...
    steps_trained_progress_bar = tqdm(
        range(total_train_steps), desc="Train steps ... ", total=total_train_steps, position=0, disable=not accelerator.is_local_main_process
    )
    ...
    if checkpoint is not None:
        accelerator.load_state(checkpoint, strict=True)
        ...
        for batch in train_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if (cur_step % training_args.save_steps == 0) or cur_step == total_train_steps:
                    ...
                    accelerator.save_state(output_dir=intermediate_dir, safe_serialization=True)
                    accelerator.wait_for_everyone()
                    if accelerator.is_main_process:
                        rotate_checkpoints(
                        ...
                        if cur_step == total_train_steps:
                            ...
                            logger.info(f"Saving final model at step {cur_step}")
