...
def generate_audio(description, text, idx, label):
    ...
    sample_rate = 44100
    sf.write(f"samples_rec/{label}_{idx}.wav", audio_arr, sample_rate)
...
mod_indah_cond = "Speaker::Indah||Reverb::very close-sounding||Noise::almost no noise||Monotony::slightly expressive and animated||Rate::slow||Pitch::moderate-pitch"
...
# for name in ['indah', 'tio']:
    # cond = "Indah" if name == 'indah' else "Tio"
    # for idx, text in tqdm(enumerate(texts), total=len(texts)):
        # os.makedirs('samples_rec', exist_ok=True)
        # generate_audio(cond, text, idx, name)

for idx, text in tqdm(enumerate(texts), total=len(texts)):
    os.makedirs('samples', exist_ok=True)
    generate_audio("indah", text, idx, "indah")
